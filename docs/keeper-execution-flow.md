...
5. **Keeper → Oracle**: `set_prices(caller, prices: Vec<SignedPrice>)` — commits a batch of ed25519-signed prices on-chain (`set_prices_simple(caller, prices: Vec<TokenPrice>)` is the test-only alternative).
...
|---|---|
| `set_prices` (Oracle) | `ORDER_KEEPER` |
